...
def setup_plot(shares, selectors, bg_var="Nothing"):
    ...
    layout = add_legend_if_necessary(p, selectors, bg_var, categories, colors)
...
def condition_plot(plot, shares, selectors, bg_var):
    ...
    if len(categories) > 1:
        ...
        legend_width = _get_legend_width(
            p.plot_width, selectors, bg_var, len(categories)
        )
...
def add_legend_if_necessary(plot, selectors, bg_var, categories, colors):
    if len(categories) == 1:
        layout = plot
    else:
        legend_entries = []
