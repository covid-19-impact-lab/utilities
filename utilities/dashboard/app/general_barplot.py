...
def prepare_data(data, variables, bg_vars, nice_names, labels, keep_last, nth_str):
    ...
    Args:
        ...
        nth_str (str): name of the "Nothing" category
...
def setup_plot(shares, selectors, bg_var, nth_str):
    ...
    layout = add_legend_if_necessary(p, selectors, bg_var, categories, colors, nth_str)

    return layout


def condition_plot(plot, shares, selectors, bg_var, nth_str):

    categories = [cat for cat in shares if cat not in NON_DATA_COLS]
    if len(categories) > 1:
        legend, p = plot.children
        legend_width = _get_legend_width(
            p.plot_width, selectors, bg_var, len(categories), nth_str
        )
    ...
    p.plot_height = get_plot_height(selectors, bg_var, nth_str)
    if bg_var == nth_str:
...
def setup_basic_plot(cds, categories, selectors, bg_var, colors, nth_str):
    f_range = FactorRange(*selectors[bg_var])
    p = figure(
        y_range=f_range,
        plot_height=get_plot_height(selectors, bg_var, nth_str),
...
def add_legend_if_necessary(plot, selectors, bg_var, categories, colors, nth_str):
    if len(categories) == 1:
        layout = plot
    else:
        legend_entries = []
        ...
        legend_width = _get_legend_width(
            plot.plot_width, selectors, bg_var, len(categories), nth_str
        )
...
def _get_legend_width(plot_width, selectors, bg_var, n_categories, nth_str):
    if bg_var == nth_str:
...
def get_plot_height(selectors, bg_var, nth_str):
    n_vars = len(selectors[nth_str])
    n_bars = len(selectors[bg_var])
    if bg_var == nth_str:
